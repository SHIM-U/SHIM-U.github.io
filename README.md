...
| 模块 | 说明 |
| --- | --- |
| 🐍 贪吃蛇小游戏 App | 产品介绍 + 游戏截图（SVG 模拟图）+ **APK 下载** |
...
├── assets/
│   ├── images/                    # 产品截图（SVG 模拟图）
│   │   ├── snake-1.svg / snake-2.svg / snake-3.svg
│   │   └── aichat.svg
│   └── downloads/
│       └── snake.apk              # 可下载的安装包
└── README.md
...
2. 本地提交并关联远程：
   ...
   git init
   git add .
   git commit -m "feat: 个人博客与产品展示站（四主题 + Pages 部署）"
   git branch -M main
...
- **更换截图**：替换 `assets/images/` 下的 SVG，或在 `index.html` 中改 `<img src>`。
